...
|---|---|---|---|
| Rédiger les cas d'utilisation détaillés (achat, vente, alerte) | `docs/use-cases-detail` | `docs: detail buy use case`, `docs: detail sell and alert use cases` | Formalise acteur/précondition/scénario/exceptions pour chaque cas |

## Phase 3 — Domaine (par module, un ticket = un module)

| Ticket | Branche | Commits | PR (résumé) |
|---|---|---|---|
| Modéliser Wallet + règles métier | `feat/wallet-domain-model` | `feat(wallet): add Wallet entity`, `feat(wallet): add can_afford rule`, `test(wallet): cover balance rules` | Entité domain pure, testée sans Django |
| Modéliser Order/Transaction + idempotence | `feat/trading-domain-model` | `feat(trading): add Order entity`, `feat(trading): add idempotency check`, `test(trading): cover double execution` | — |
